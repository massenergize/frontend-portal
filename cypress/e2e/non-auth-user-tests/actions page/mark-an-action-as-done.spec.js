...
describe("Mark an action as done", function () {
    ...
    it('Marks an action as done or undone', () => {
        ...
        cy.get('span[style="font-size: 55px; font-weight: 700; color: black;"]').eq(0).invoke('text').then((initialText) => {
            ...
            cy.get('#test-action-cards-wrapper').children().then(($elements) => {
                ...
                if (itemCount > 0) {
                    cy.get(`#test-action-cards-wrapper > :nth-child(${getRandomInt(itemCount)})`).contains('Done').click()
                    ...
                    cy.get('span[style="font-size: 55px; font-weight: 700; color: black;"]').eq(0).invoke('text').then((finalText) => {
