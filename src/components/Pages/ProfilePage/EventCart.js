      ...
      <div className="cart-outer mb-5">
        ...
        <div className="table-outer">
          <table className="cart-table" style={{ width: "100%" }}>
            ...
            <tbody>
              {this.props.info
                ? this.renderEventsMoreInfo(this.props.eventRSVPs)
                : this.renderEvents(this.props.eventRSVPs)}
            </tbody>
          </table>
        </div>
      </div>
  ...
  renderEvents(eventRSVPs) {
    if (!eventRSVPs || eventRSVPs.length <= 0) {
      return (
        <tr key="1">
          <td colSpan="100%">
            <p className="m-0 p-2 w-100 text-center">
              Nothing here, yet! See all{" "}
              <Link to={this.props.links.events}> events </Link>
            </p>
          </td>
        </tr>
      );
    ...
    if (!eventRSVPs || eventRSVPs.length <= 0) {
      return (
        <tr key="1">
          <td colSpan="100%">
            <p
              className="m-0 p-2 w-100 text-center"
              style={{ fontSize: "1rem" }}
            >
              Nothing here, yet! See all{" "}
              <Link to={this.props.links.events}> events </Link>
      ...
      return (
        <tr key={key}>
          ...
          <td className="prod-column">
            <div className="column-box">
              <p>
                {this.sameDay(startdate, enddate)
